...
create or replace function sqlr2r.bash (
...
values  sqlr2r.bash ('cd /home;ls'); 

-- more readable:
select * from table(systools.split (sqlr2r.bash ('cd /home;ls'),x'25')); 


-- ensure NLS works
values  sqlr2r.bash ('echo "æøå" '); 

-- No issues with multiple calls for each row
select 
    authorization_name, 
    home_directory  , 
    sqlr2r.bash ('cd ' || home_directory || ';ls -xla') dir_list
